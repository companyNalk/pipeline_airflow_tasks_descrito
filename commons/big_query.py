...
# # #
# # #     def _is_time(self, v):
# # #         """Verifica se o valor é um horário válido."""
# # #         # Regex mais restritivo que valida semanticamente os valores
# # #         # Formato: HH:MM:SS ou H:MM:SS (opcionalmente com milissegundos)
# # #         pattern = r"^([01]?\d|2[0-3]):([0-5]\d):([0-5]\d)(\.\d+)?$"
# # #
# # #         if not re.match(pattern, v):
# # #             return False
# # #
# # #         # Validação adicional opcional para maior segurança
# # #         try:
# # #             parts = v.split(':')
# # #             hours = int(parts[0])
# # #             minutes = int(parts[1])
...
# # #         """Detecta e classifica o tipo de data no valor."""
# # #         try:
...
# # #
# # #
# # # # Função worker para processamento paralelo de colunas
# # # def analyze_column_worker(column_name, column_data, boolean_values, type_mapping):
# # #     """Worker function para análise paralela de colunas."""
# # #
# # #     def detect_pattern(value, boolean_values):
# # #         """Função auxiliar para detectar padrões."""
# # #         if not isinstance(value, str):
# # #             return 'string'
# # #
# # #         value_lower = value.lower().strip()
...
# # #
# # #         # Verificar se é hora
# # #         if bool(re.match(r"^(\d{1,2}:)?\d{1,2}:\d{2}(\.\d+)?$", value)):
# # #             return 'time'
# # #
# # #         # Detecção de data
...
# #                 logger.info("Usando credenciais de Authorized User (OAuth2)")
# #             else:
