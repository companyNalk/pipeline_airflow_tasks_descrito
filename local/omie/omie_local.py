...
This module contains functions specific to the data extraction from Omie API.
Modified to run locally without GCP integration.
"""
...
def run(customer):
    ...
    OUTPUT_DIR = customer.get('output_dir', 'omie_data')

    # Create output directory if it doesn't exist
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # Create subdirectories for each endpoint
    endpoints = [
        'listar_categorias', 'listar_produtos_pedidos', 'vendedores',
        'listar_produtos', 'listar_motivo_devolucao', 'listar_pedidos_etapas',
        'etapafat', 'pedidos_vendas'
    ]
    for endpoint in endpoints:
        os.makedirs(os.path.join(OUTPUT_DIR, endpoint), exist_ok=True)
    ...
    def save_to_local_csv(df, local_path, sep='|'):
        """Salva DataFrame para CSV localmente"""
        if df.empty:
            logger.warning("DataFrame vazio, salvamento cancelado.")
            return False
        try:
            ...
            # Salva o arquivo
            df.to_csv(local_path, sep=sep, index=False, quoting=1)
            ...
            # hist_path = os.path.join(dir_path, "historico", f"{filename_base}_{timestamp}.csv")
            # os.makedirs(os.path.join(dir_path, "historico"), exist_ok=True)
            # df.to_csv(hist_path, sep=sep, index=False, quoting=1)
    ...
    def coletar_categorias():
        ...
        save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/categorias_omie.csv"))
    ...
    def coletar_pedidos():
        ...
        df = pd.DataFrame(resultados)
        save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/pedidos_omie.csv"), sep='|')
    ...
    def coletar_vendedores():
        ...
        if resultados:
            df = pd.DataFrame(resultados)
            save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/vendedores.csv"))
    ...
    def coletar_produtos():
        ...
        if resultados:
            df = pd.DataFrame(resultados)
            save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/produtos.csv"))
    ...
    def coletar_motivos_devolucao():
        ...
        if motivos:
            ...
            save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/motivos_devolucao.csv"))
    ...
    def coletar_etapas_pedido():
        ...
        if resultados:
            df = pd.DataFrame(resultados)
            save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/etapas_pedidos.csv"))
    ...
    def coletar_etapas_faturamento():
        ...
        if resultados:
            df = pd.DataFrame(resultados)
            save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/etapas_faturamento.csv"))
            logger.info(f"Coleta finalizada em {datetime.now() - inicio}")
    ...
    def coletar_pedidos_venda(c_etapa="50"):
        ...
        if pedidos:
            ...
            save_to_local_csv(df, os.path.join(OUTPUT_DIR, f"{ENDPOINT_NAME}/pedidos_venda_etapa_{c_etapa}.csv"))
...
# Exemplo de como iniciar o script
if __name__ == "__main__":
    # Configurações necessárias para executar
    customer = {
        'api_key': '50691949308',  # Substitua com sua API key
        'api_secret': '30ad08cd430eadf0495842bf979307f8',  # Substitua com seu API secret
