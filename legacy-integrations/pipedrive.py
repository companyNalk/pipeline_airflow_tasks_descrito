...
Pipedrive module for data extraction functions.
This module contains functions specific to the Pipedrive integration.
Version: 2.1 - Added JSON processing for sdr_responsavel and closer_responsavel columns
"""
...
def run(customer):
    ...
    def process_responsavel_json_columns(df: pd.DataFrame) -> pd.DataFrame:
        """
        Processa as colunas sdr_responsavel e closer_responsavel que contêm dados JSON,
        extraindo as chaves para novas colunas.

        Args:
            df: DataFrame com os dados dos deals

        Returns:
            DataFrame com as novas colunas extraídas dos JSONs
        """
        import json

        df_processed = df.copy()

        # Colunas que contêm JSON para processar
        json_columns = ['sdr_responsavel', 'closer_responsavel']

        for col in json_columns:
            if col in df_processed.columns:
                print(f"Processando coluna JSON: {col}")

                # Identificar quais chaves existem nos JSONs desta coluna
                all_keys = set()
                ...
                for idx, value in df_processed[col].items():
                    if pd.notna(value) and value != '' and value != 'None':
                        try:
                            ...
                            # Coletar todas as chaves do JSON
                            if isinstance(json_data, dict):
                                all_keys.update(json_data.keys())

                        except (json.JSONDecodeError, TypeError, ValueError):
                            # Se não conseguir fazer parse, pular
                            continue

                # Criar novas colunas para cada chave encontrada
                for key in all_keys:
                    new_col_name = f"{col}_{key}"
                    df_processed[new_col_name] = None

                    # Extrair o valor de cada registro
                    for idx, value in df_processed[col].items():
                        if pd.notna(value) and value != '' and value != 'None':
                            try:
                                # Fazer parse do JSON
                                ...
                                # Extrair o valor da chave específica
                                if isinstance(json_data, dict) and key in json_data:
                                    df_processed.at[idx, new_col_name] = json_data[key]

                            except (json.JSONDecodeError, TypeError, ValueError):
                                # Se não conseguir fazer parse, deixar None
                                continue

                print(f"  Criadas {len(all_keys)} novas colunas para {col}: {list(all_keys)}")
    ...
    def main():
        ...
        # PROCESSAR COLUNAS JSON DE RESPONSÁVEIS
        print("\nProcessando colunas JSON de responsáveis...")
        deals_df = process_responsavel_json_columns(deals_df)
...
def run_persons(customer):
    ...
    def fix_hash_columns(df: pd.DataFrame, field_mappings: dict = {}) -> pd.DataFrame:
        # Detecta colunas cujo nome parece um hash SHA-1 (40 caracteres hexadecimais)
        hash_cols = [col for col in df.columns if re.match(r'^[0-9a-f]{40}$', col)]
