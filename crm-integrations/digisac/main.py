...
def get_arguments():
    ...
    return (ArgumentManager("Script para coletar e processar dados da API Digisac")
            .add("API_BASE_URL", "URL base", required=True, default="https://expotelecom.digisac.biz")
...
def fetch_page(endpoint, token, page=1):
    ...
    try:
        ...
        # Para o endpoint answers/overview que tem estrutura diferente
        if endpoint == "answers/overview":
            # Este endpoint não é paginado, retorna todos os dados de uma vez
            return {
        ...
        # Para endpoints paginados (questions e contacts)
        items = data.get('data', [])
...
def fetch_all_pages(endpoint, token):
    ...
    # Caso específico para o endpoint answers/overview que não é paginado
    if endpoint == "answers/overview":
        try:
            data = fetch_data(endpoint, token)
            logger.info(f"✓ Endpoint {endpoint}: dados obtidos com sucesso")
            return [data]  # Retorna o resultado em uma lista para manter consistência
        except Exception as e:
...
def main():
    ...
    # Adicionar o sufixo /api/v1/ à URL base conforme o exemplo fornecido
